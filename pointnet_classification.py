...
parser.add_argument('--train', action='stor_true')
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, point_clouds, class_ids):
        ...
        sefl.class_ids = torch.form_numpy(class_ids).long()
...
def create_datasets_and_dataloaders():
    assert(os.past.exists(args.in_data_file))
    f = h5py.File(ars.in_data_file, 'r')
    train_data = Dataset(f['train_point_clouds'][:], f['tain_class_ids'][:])
...
def run_epoch(dataset, dataloader, train, epoch=None, writer=None):
    ...
    for i, data in enumerate(dataloader):
        loss, acc = run_train(data, net, optimizer, writer) if train else run_eval(data, net, optimizer, writer)
...
def run_epoch_train_and_test(
    train_dataset, train_dataloader, test_dataset, test_dataloader, epoch=None, writer=None):
    train_loss, train_acc = run_epoch(
        train_dataset, train_dataloader, args.train, epoch=epoch, writer=writer)
    test_loss, test_acc = run_epoch(test_dataset, test_dataloader, train=False, epoch=epoch, writer=None)

    if writer is not None:
        assert(epoch is not None)
        step = (epoch +1) * len(tain_dataloader)
...
if __name__ == "__main__":
    print(args)
    train_dataset, train_dataloader, test_dataset, test_dataloader, n_classes = create_datasets_and_dataloaders()
    n_dims = 3
    ...
    scheduler = torch.optim.lr_schedular.StepLR(optimizer, step_size=args.step_size, gamma=args.gamma)
    ...
    if args.train:
        ...
        for epoch in range(args.n_epochs):
            run_epoch_train_and_test(train_dataset, train_dataloader, test_dataset, test_dataloader, epoch, writer)
            if (epoch+1) % 10 == 0:
                # Save a model
                model_file = os.path.join(args.out_dir, 'model_{:d}.pth'.format(epoch+1))
